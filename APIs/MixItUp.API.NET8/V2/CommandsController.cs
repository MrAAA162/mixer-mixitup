...
namespace MixItUp.API.NET8.V2
{
    ...
    public class CommandsController : ControllerBase
    {
        ...
        public ActionResult<GetSingleCommandResponse> GetCommandById(Guid commandId)
        {
        ...
        [HttpGet]
        public ActionResult<GetListOfCommandsResponse> GetAllCommands([FromQuery] int skip = 0, [FromQuery] int pageSize = 25)
        {
            var allCommands = GetAllCommandsInternal();
        ...
        [HttpPatch("{commandId:guid}/state/{state:int}")]
        public async Task<ActionResult<GetSingleCommandResponse>> UpdateCommandState(Guid commandId, CommandStateOptions state)
        {
            if (!ChannelSession.Settings.Commands.TryGetValue(commandId, out var command) || command == null)
            {
                return NotFound();
        ...
        private IEnumerable<CommandModelBase> GetAllCommandsInternal()
        {
